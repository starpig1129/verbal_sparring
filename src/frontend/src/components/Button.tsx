...
const styles: Record<ButtonVariant, string> = {
  ...
  'secondary': 'border border-bamboo text-[#a88a6d] hover:text-white hover:border-[#886655] bg-[#14100b] hover:shadow-[0_0_10px_rgba(136,102,85,0.15)] disabled:opacity-40 disabled:cursor-not-allowed',
}

export default function Button({ variant = 'primary-outline', className = '', children, ...props }: Props) {
  return (
    <button
      className={`font-display font-black tracking-[3px] uppercase text-[11px] px-4 py-2 transition-all duration-100 transform active:scale-95 ${styles[variant]} ${className}`}
      {...props}
    >
      {children}
    </button>

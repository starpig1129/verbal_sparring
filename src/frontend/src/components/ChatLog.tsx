  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-3 bg-[#060502]" style={{ minHeight: 0 }}>
      ...
      {visible.map((e) => {
        ...
        if (e.kind === 'attack') return <MessageBubble key={e.id} kind="attack" sender={e.sender} displayText={e.displayText} damage={e.damage} isNpc={e.isNpc} isPending={e.isPending} isCrit={e.isCrit} combo={e.combo} myUsername={myUsername} />
